...
const Categories = () => {
  ...
  //   try {
  //     await apiRequest(`${API_BASE_URL}/challenge-creation/${categoryId}`, {
  //       method: "DELETE",
  //     });
  //   } catch (err) {
  //     throw new Error(err.message);
  ...
  // const incrementCategoryCount = async (categoryId) => {
  //   try {
  //     const response = await apiRequest(
  //       `${API_BASE_URL}/challenge-creation/${categoryId}/increment`,
  //       {
  //         method: "PUT",
  //       }
  //     );
  ...
  const incrementCount = async (categoryId) => {
    try {
      ...
      setCategories((prev) =>
        prev.map((cat) =>
          cat.id === categoryId ? { ...cat, count: updatedCategory.count } : cat
        )
      );
    } catch (err) {
